...
const algorithmData = {
  algChoice: [
    ref("1．下列说法错误的是（     ）。\nA．算法是程序在计算机上的具体实现。\nB．算法是用计算机编程语言翻译的结果。\nC．算法能直接在计算机上执行。\nD．程序能直接在计算机上执行。", "该题表述本身不够严谨。若按单选题处理，通常优先选 C。算法是解决问题的步骤描述，不能直接在计算机上执行；程序才是算法在计算机上的实现，可以直接执行。", [], { excluded: true }),
    ...
    ref("13．T(n)=2T(n/2)+1，T(1)=1，则 T(n)=（     ）。\nA．nlogn    B．logn    C．n^2    D．nlgn", "由递推展开可得 T(n)=2n-1，因此渐进时间复杂度为 Θ(n)。原选项中没有标准正确项。", [], { excluded: true }),
  ...
  algBlank: [
    ...
    ref("3．回溯法算法框架中定义问题的解空间时，解的形式是 ______；解空间结构的组织形式通常有 ______、______、______。", "答案：n 元组；子集树、排列树、满 m 叉树。"),
    ref("4．回溯方法搜索问题的方式是 ______；剪枝函数和限界函数的作用分别是 ______。", "答案：深度优先；剪枝函数用于剪去不可能产生可行解的分支，限界函数用于剪去不可能优于当前最优解的分支。"),
    ref("5．用 A[1:n] 表示 A1…An 连乘，给定 A[1:10]，用动态规划算法求解时，用来存储各子问题最优值的二维数组共有 ______ 个存储单元。", "答案：100 个。若只按有效上三角子问题计算，则为 55 个。"),
    ref("6．算法是由若干条指令组成的有穷序列，且满足输入、输出、______、______ 性和能行性共 5 条性质。", "答案：确定性、有限性。"),
    ref("7．用二分搜索算法在一个有序序列 a[1:n] 中查找某指定元素 x 是否存在。在查找过程中，若当前查找范围是 a[low:high]，则分治策略中的分解操作是 ______。", "答案：取中间位置 mid，把查找区间分成 low..mid-1 和 mid+1..high 两个子区间。"),
    ref("8．用 A[1:n] 表示 A1…An 连乘，用动态规划求解 A[1:n] 时，计算各子问题最优值的复杂度是 ______。", "答案：O(n^3)。"),
    ref("9．贪心法的两个基本要素是 ______ 和 ______。", "答案：贪心选择性质、最优子结构。"),
    ref("10．分治法解题的三个步骤是 ______、______、______，分治算法解决问题的核心思想是 ______。", "答案：分解、求解、合并；把大问题分解为与原问题同类的小问题，递归求解后再合并。"),
    ref("11．分支限界法是以 ______ 或 ______ 的方式进行搜索问题的解空间树。", "答案：广度优先、最小耗费优先或最大效益优先。", [], { excluded: true }),
    ...
    ref("14．回溯法是从根结点开始，以 ______ 进行搜索解空间树；分支限界法是从根结点开始，以 ______ 进行搜索解空间树。", "答案：深度优先；广度优先或优先队列式搜索。"),
    ref("15．使用算法 LCS 找两个序列 A=\"xzyzzyx\" 和 B=\"zxyyzxz\" 的最长公共子序列。它的一个最长公共子序列为 ______，长度是 ______。", "答案：zyzx，4。也可能存在其他长度为 4 的最长公共子序列。"),
...
<pre class="answer-code">void Backtrack(int t)
{
    ...
    ref("18．动态规划的求解步骤是：最优子结构性质分析、______、计算最优值并记录相关信息，根据 ______。", "答案：建立递归关系或状态转移方程；记录的信息构造最优解。"),
...
若 Xi = Yj，则 c[i][j] = c[i-1][j-1] + 1
若 Xi ≠ Yj，则 c[i][j] = max(c[i-1][j], c[i][j-1])</pre>
<p>把两个序列代入表格计算后，可得它们的最长公共子序列长度为 <strong>3</strong>。一个最长公共子序列可以写成 <strong>ADB</strong>，也可以写成 <strong>AHB</strong>。</p>`,
      answerImages: ["public/imgs/alg-4-11-state.png"]
    }),
