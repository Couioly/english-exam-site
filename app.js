...
const algorithmData = {
  algChoice: [
    ref("1．下列说法错误的是（     ）。\nA．算法是程序在计算机上的具体实现。\nB．算法是用计算机编程语言翻译的结果。\nC．算法能直接在计算机上执行。\nD．程序能直接在计算机上执行。", "按当前给定答案键记作 A、B、C。若从概念上理解，算法是解决问题的步骤描述，不能直接在计算机上执行；程序才是算法在计算机上的实现，可以直接执行。", [], { excluded: true }),
    ...
    ref("13．T(n)=2T(n/2)+1，T(1)=1，则 T(n)=（     ）。\nA．nlogn    B．logn    C．n^2    D．nlgn", "按当前给定答案键记作 A、D。若按递推式展开，实际可得 T(n)=2n-1，渐进复杂度为 Θ(n)。", [], { excluded: true }),
    ...
    ref("15．无向图 G=(V,E)，|V|=n，图的 m 可着色判定问题时间复杂度为（     ）。\nA．O(mn^m)    B．O(nm^n)    C．O(2^mn)    D．O(2^nm)", "图的 m 可着色问题通常用回溯法求解，最坏情况下需要枚举 m^n 种着色方案，并在每层进行约束检查，所以时间复杂度通常写作 O(nm^n)。", [], { excluded: true }),
    ref("16．两个序列长度分别为 m、n，求最长公共子序列耗时（      ）。\nA．O(m^2)    B．O(n^2)    C．O(mn)    D．O(mn)", "答案：C、D。原题两个正确选项内容相同，标准复杂度为 O(mn)。"),
    ref("17．影响算法时间复杂度的主要因素除算法本身外，正确的是（多选）。\nA．一定与输入序列有关\nB．一定不与输入序列有关\nC．不一定与输入序列有关\nD．一般与问题规模有关", "算法时间复杂度除与算法本身有关外，还与问题规模有关；至于是否和输入序列有关，要看具体问题，因此选 C、D。", [], { excluded: true }),
    ...
    ref("25．关于算法时间复杂度主要影响因素，不正确的是（多选）。\nA．一定与输入序列有关\nB．一定不与输入序列有关\nC．不一定与输入序列有关\nD．一般与问题规模有关", "不正确的是 A、B。算法时间复杂度不一定与输入序列有关，但一般与问题规模有关。", [], { excluded: true }),
    ...
    ref("47．伪代码中常用符号“=”表示（   ）。\nA．等于\nB．小于等于\nC．赋值\nD．交换", "答案：C。"),
    ref("48．哪个问题适合动态规划？哪个适合分治？哪个适合贪心？哪个适合回溯？\nA．N皇后    B．棋盘覆盖    C．部分背包    D．矩阵链乘法", "答案：D、B、C、A。即动态规划对应 D，分治对应 B，贪心对应 C，回溯对应 A。")
  ],
  algBlank: [
    ref("1．算法复杂性是指算法在计算机上运行时所消耗的计算机资源的量。最重要的计算机资源是 ______ 和 ______。", "答案：时间、空间。"),
    ref("2．二分搜索算法在最坏情况下的时间复杂度是 ______。", "答案：O(log n)。"),
    ref("3．回溯法算法框架中定义问题的解空间时，解的形式是 ______；解空间结构的组织形式通常有 ______、______、______。", "答案：n元组；子集树、排列树、m叉树。"),
    ref("4．回溯方法搜索问题的方式是 ______；剪枝函数和限界函数的作用分别是 ______。", "答案：深度优先搜索；剪枝函数用于提前舍弃不满足约束条件的子树，限界函数用于剪去得不到最优解的子树。"),
    ref("5．用 A[1:n] 表示 A1…An 连乘，给定 A[1:10]，用动态规划算法求解时，用来存储各子问题最优值的二维数组共有 ______ 个存储单元。", "答案：100 个。若只按有效上三角子问题计算，则为 55 个。"),
    ref("6．算法是由若干条指令组成的有穷序列，且满足输入、输出、______、______ 和能行性共 5 条性质。", "答案：确定性、有穷性。"),
    ref("7．用二分搜索算法在一个有序序列 a[1:n] 中查找某指定元素 x 是否存在。在查找过程中，若当前查找范围是 a[low:high]，则分治策略中的分解操作是 ______。", "答案：取中间位置 mid=[(low+high)/2]，将区间拆分为 [low, mid-1]、[mid]、[mid+1, high] 三部分。"),
    ref("8．用 A[1:n] 表示 A1…An 连乘，用动态规划求解 A[1:n] 时，计算各子问题最优值的复杂度是 ______。", "答案：O(n^3)。"),
    ref("9．贪心法的两个基本要素是 ______ 和 ______。", "答案：最优子结构性质、贪心选择性质。"),
    ref("10．分治法解题的三个步骤是 ______、______、______，分治算法解决问题的核心思想是 ______。", "答案：分解子问题、递归求解、合并子问题的解；递归。"),
    ref("11．分支限界法是以 ______ 或 ______ 的方式进行搜索问题的解空间树。", "答案：广度优先、最大效益优先。", [], { excluded: true }),
    ref("12．分治法的思想是将原问题分成 k（k>1）个子问题，子问题之间 ______ 且与原问题相同，用 ______ 方法求解子问题。", "答案：相互独立；递归。"),
    ref("13．最大团问题的解空间树是 ______ 树，旅行商问题的解空间树是 ______ 树。", "答案：子集树；排列树。"),
    ref("14．回溯法是从根结点开始，以 ______ 进行搜索解空间树；分支限界法是从根结点开始，以 ______ 进行搜索解空间树。", "答案：深度优先DFS；广度优先BFS。"),
    ref("15．使用算法 LCS 找两个序列 A=\"xzyzzyx\" 和 B=\"zxyyzxz\" 的最长公共子序列。它的一个最长公共子序列为 ______，长度是 ______。", "答案：xyzx，4。"),
    ref("16．用分治法设计的合并排序算法描述如下，其中 A[low:high] 存放待排序序列，请填写空缺部分的语句。", "见完整代码", [], {
      answerHtml: `<p>两处空缺分别是 <code>middle=(low+high)/2</code> 和 <code>MergeSort(A,middle+1,high)</code>，补全后的代码如下：</p>
<pre class="answer-code">void MergeSort(int A[], int low, int high)
{
    int middle;
    if (low &lt; high)
    {
    ...
    ref("17．给定 n 个顶点的无向图和 m 种颜色，图的 m 可着色问题描述如下，其中 t 为当前扩展结点所处层次，OK 函数用来判断是否可以着相应颜色，请填写空缺部分的语句。", "见完整代码", [], {
      excluded: true,
      answerHtml: `<p>两处空缺分别是 <code>x[t]=i</code> 和 <code>Backtrack(t+1)</code>，补全后的代码如下：</p>
<pre class="answer-code">void Backtrack(int t)
{
    if (t &gt; n)
        sum++;
    else
    ...
    ref("18．动态规划的求解步骤是：最优子结构性质分析、______、计算最优值并记录相关信息，根据 ______。", "答案：建立最优值的状态转移方程；记录的信息构造最优解。"),
    ref("19．给定 n 个城市的交通图，住地城市为 1 号城市，旅行售货员问题的解空间树是 ______，该问题回溯法的时间复杂度为 ______。", "答案：排列树；O(n!)。"),
    ref("20．n 个物品、背包容量为 V 的 0-1 背包问题中，设第 i 个物品的价值为 pi、重量为 vi，则状态转移方程的递推公式是 ______。", "答案：m[i][j]=m[i-1][j]（j<vi）；m[i][j]=max(m[i-1][j], m[i-1][j-vi]+pi)（j≥vi）。"),
    ref("21．表示渐近紧确界的符号是（    ）。", "答案：Θ。"),
    ref("22．计算斐波那契数列第 n 项值的问题，如果采用动态规划算法求解，状态转移公式是 ______。", "答案：F(0)=0，F(1)=1，F(n)=F(n-1)+F(n-2)。"),
    ref("23．动态规划的基本要素有 ______、______、自底向上的求解方法。", "答案：最优子结构、重叠子问题。"),
    ref("24．给定 n 个顶点的无向图和 m 种颜色，图的 m 可着色问题的解空间树是 ______，该问题回溯法的时间复杂度为 ______。", "答案：满 m 叉树；O(nm^n)。"),
    ref("25．某算法的计算时间 T(n) 满足递归关系式 T(n)=2T(n/2)+1，n>1；T(1)=1。则 T(n)= ______。", "答案：2n-1，渐进复杂度为 Θ(n)。")
  ],
  algShort: [
    ref("1．简述算法的概念及特征，算法伪代码的具体符号表示含义。", "算法是为解决某一类问题而规定的一个有穷操作序列。算法具有五个基本特征：输入、输出、确定性、有限性和可行性。伪代码是一种介于自然语言和程序语言之间的描述工具，其中常见符号含义是：`=` 表示赋值，`if...then...else` 表示条件判断，`for`、`while` 表示循环，`()` 常用于函数或过程调用，数组下标表示访问对应元素。"),
    ref("2．什么是算法复杂性？如何衡量？", "算法复杂性是指算法在运行时所消耗的资源数量，主要包括时间复杂度和空间复杂度。衡量时通常把问题规模记为 n，分析当 n 增大时算法执行时间和存储空间的变化趋势，并用 O、Ω、Θ 等渐进符号表示。"),
    ref("3．简述分治法的特征、思想和适用条件。", "分治法的基本思想是：把一个规模较大的问题分解成若干个规模较小、与原问题同类的子问题，递归求解后再把子问题的解合并成原问题的解。它的特点是分解、递归、合并。适用条件是：原问题能够分解，子问题与原问题同型，子问题之间相互独立，且子问题的解能够合并。"),
    ref("4．原文该题仅保留题号，未提供具体题干。", "原题在资料中未给出具体内容，当前无法整理成标准答案，复习时可先跳过。"),
    ref("5．简述贪心算法的基本思想和解题步骤、需要满足的基本要素。", "贪心算法的基本思想是：从问题的某一个初始解出发，每一步都作出当前看来最优的局部选择，希望最后得到全局最优解。解题步骤一般是：建立候选集合，确定贪心策略，从候选集合中选出当前最优元素，判断加入后是否可行，若可行则加入解集合，直到形成完整解。贪心算法需要满足两个基本要素：贪心选择性质和最优子结构。"),
    ref("6．简述动态规划的求解步骤。为什么说动态规划算法是以空间换取时间？", "动态规划的求解步骤是：先分析问题是否具有最优子结构，再定义状态，建立状态转移方程，确定边界条件，自底向上计算最优值并记录相关信息，最后根据记录的信息构造最优解。之所以说动态规划是以空间换时间，是因为它把已经求出的子问题结果保存下来，避免重复计算，从而用额外存储空间换取更高的时间效率。"),
    ref("7．简述动态规划算法的基本要素。", "动态规划的基本要素主要有两个：最优子结构和重叠子问题。此外，在具体求解时还要明确状态定义、状态转移方程、边界条件，并采用自底向上的方法求解。"),
    ref("8．简述动态规划算法与分治算法的异同。", "动态规划和分治法的相同点是：都把原问题分解为若干个子问题来求解。不同点是：分治法的子问题通常相互独立，递归求解后再合并；动态规划的子问题通常存在重叠，因此需要把子问题结果保存起来，避免重复计算，常采用自底向上的方式求解。"),
    ref("9．简述回溯算法的思想、可行解、可能解和最优解的概念以及关系。", "回溯算法的思想是：从根结点出发，按照深度优先方式搜索解空间树，逐步构造问题的解；当发现当前结点不满足约束条件，或继续向下搜索也不可能得到更优解时，就停止向下扩展并退回到上一个结点继续搜索。可能解是解空间中的候选解；可行解是满足约束条件的可能解；最优解是在所有可行解中目标函数值最优的解。也就是说，最优解一定是可行解，可行解一定属于可能解。")
  ],
  algDesign: [
    ref("1．活动安排问题，算法贪心策略，伪代码。", "见标准答案", ["public/imgs/4-1.png"], {
      answerHtml: `<p>活动安排问题采用贪心法。贪心策略是<strong>按活动结束时间从小到大排序，每次选择当前结束最早且与已选活动相容的活动</strong>。</p>
<p>这样做的原因是：结束越早，给后续活动留下的可选时间越多，因此更有利于得到最多的相容活动。</p>
<pre class="answer-code">算法：Activity-Selection(s, f, n)
输入：n 个活动的开始时间 s 和结束时间 f
输出：一个最大相容活动集合 S
按结束时间 f 从小到大排序
S ← {a1}
last ← 1
...
return S</pre>`
    ...
    ref("2．按分治策略求解棋盘覆盖问题时，对于如下图所示的 2^3×2^3 的特殊棋盘，共需要多少个 L 型骨牌；并在棋盘上填写 L 型骨牌的覆盖情况。", "见标准答案", [], {
      answerHtml: `<p>8×8 棋盘共有 64 个方格，去掉 1 个特殊方格后还剩 63 个方格；每个 L 型骨牌覆盖 3 个方格，因此一共需要 <strong>21</strong> 个 L 型骨牌。</p>
<p>求解时采用分治法：先把原棋盘分成四个 4×4 子棋盘。原来的特殊方格只在其中一个子棋盘里，再在中心位置放一个 L 型骨牌，使另外三个子棋盘各自产生一个“特殊方格”。这样四个子棋盘都变成同类的特殊棋盘覆盖问题，然后递归处理即可。</p>`,
      answerImages: ["public/imgs/alg-4-2.jpg"]
    }),
    ref("3．选第 K 小元素，问题分析，伪代码。", "见标准答案", ["public/imgs/4-3.png"], {
      answerHtml: `<p>可先递归求当前序列中的最小元素及其对应的淘汰表，再重复执行 k 次，最后一次取出的元素就是第 k 小元素。</p>
<p>具体做法是：把序列分成左右两半分别求最小值，较小者保留，较大者记入较小者的淘汰表；不断从当前序列中取最小值，执行到第 k 次时返回结果。</p>
<pre class="answer-code">算法：find_min(a_list)
输入：n 个元素序列
输出：(当前序列最小元素, 该最小值的专属淘汰列表 lose_list)
n ← length(a_list)
if n == 1 then
  return (a_list[0], 空列表)
if n>1 then
  mid ← n // 2
  (left_min, left_lose) ← find_min(a_list[0:mid])
  (right_min, right_lose) ← find_min(a_list[mid:n])
  if left_min &lt; right_min then
    将 right_min 插入 left_lose 中
    return (left_min, left_lose)
  else
    将 left_min 插入 right_lose 中
    return (right_min, right_lose)
...
current_list ← arr
for i ← 1 to k do
  (min_result, current_list) ← find_min(current_list)
return min_result</pre>`
    }),
    ref("4．最长公共子序列，问题分析，状态转移方程的书写，备忘录填充。", "见标准答案", ["public/imgs/4-4.png"], {
      answerHtml: `<p>设 <code>c[i][j]</code> 表示序列 <code>X[1..i]</code> 与 <code>Y[1..j]</code> 的最长公共子序列长度，则状态转移方程为：</p>
<pre class="answer-code">当 i = 0 或 j = 0 时，c[i][j] = 0
若 Xi = Yj，则 c[i][j] = c[i-1][j-1] + 1
若 Xi ≠ Yj，则 c[i][j] = max(c[i-1][j], c[i][j-1])</pre>
<p>备忘录填充时，先把第 0 行和第 0 列全部置为 0，再按行或按列自底向上填表。</p>
<pre class="answer-code">算法：Longest-Common-Subsequence(X, Y)
输入：两个子序列 X, Y
输出：LCS 长度表 C 和追踪表 rec
n ← length(X), m ← length(Y)
建立数组 C[0..n,0..m] 和 rec[0..n,0..m]
...
return C, rec</pre>`,
      answerImages: ["public/imgs/alg-4-11-state.png"]
    }),
    ref("5．循环赛日程表的安排问题。n=2^3 个选手的比赛日程表的制定。", "见标准答案", ["public/imgs/4-5.png"], {
      answerHtml: `<p>8 个选手进行单循环比赛，每个选手都要和其余 7 个选手各赛一场，所以共需要 <strong>7 天</strong>。该题采用分治法构造赛程表。</p>
<p>先安排前 4 个选手的赛程，再将结果复制到另外两个对应区域，并加上编号偏移，这样就能保证每名选手每天都遇到不同的对手。</p>
<pre class="answer-code">算法：arrange(p, q, n, arr)
输入：子问题位置 (p, q)，规模 n
输出：循环赛日程表 arr
if n &gt; 1 then
  arrange(p, q, n/2, arr)
...
return arr</pre>`
    ...
    ref("6．动态规划算法求解 01 背包问题，备忘录填空，伪代码书写。", "见标准答案", [], {
      answerHtml: `<p>设 <code>P[i,c]</code> 表示前 <code>i</code> 个物品装入容量为 <code>c</code> 的背包时所能取得的最大总价值。</p>
<p>若不选第 i 个物品，则值为 <code>P[i-1,c]</code>；若选第 i 个物品，则值为 <code>P[i-1,c-w[i]]+v[i]</code>；两者取最大值。</p>
<pre class="answer-code">算法：KnapsackMR(i, c)
输入：商品集合 {1, …, i}，背包容量 c
输出：最大总价值 P[i,c]
if c &lt; 0 then
  return -∞
...
P1 ← KnapsackMR(i-1, c-v[i])
P2 ← KnapsackMR(i-1, c)
P[i,c] ← max(P1+pi, P2)
return P[i,c]</pre>
<p>这道题的关键在于“选或不选当前物品”，并用备忘录保存已经求出的子问题结果，避免重复计算。</p>`
    }),
    ref("7．贪心算法解题的步骤中，各函数的含义。", "见标准答案", [], {
      answerHtml: `
<p><code>A</code> 表示候选集合；<code>S</code> 表示当前已构造的解集合；<code>solution(S)</code> 判断 <code>S</code> 是否已经构成完整解；<code>select(A)</code> 按贪心策略从候选集合中选出当前看来最优的元素；<code>feasible(S, x)</code> 判断把元素 <code>x</code> 加入 <code>S</code> 后是否仍满足约束条件。</p>
<pre class="answer-code">S ← ∅
while not solution(S) do
  x ← select(A)
  if feasible(S, x) then
    S ← S ∪ {x}
  A ← A - {x}
return S</pre>
<p>贪心算法的核心是：每一步都做当前看来最优的局部选择，希望最终得到全局最优解。</p>`
    }),
    ref("8．用回溯法解 4 皇后问题（要求：指明搜索范围，解空间结构，搜索时用的剪枝函数，并画出搜索树）。", "见标准答案", [], {
      answerHtml: `<p>设 <code>x[i]</code> 表示第 <code>i</code> 行皇后所在的列号，则 4 皇后问题的解可表示为 <code>x=(x1,x2,x3,x4)</code>。搜索范围是：每一层确定一行皇后的位置，每行都可在 1 到 4 列中选择，所以按行向下搜索。</p>
<p>由于每行只放一个皇后，并且列号不能重复，因此该问题的解空间可组织为<strong>排列树</strong>。搜索方式采用深度优先。</p>
<p>剪枝函数为：若当前皇后与前面某个皇后同列，或在同一条对角线上，则该结点不能继续向下扩展。即当 <code>x[i]=x[j]</code> 或 <code>|i-j|=|x[i]-x[j]|</code> 时剪枝。</p>
<pre class="answer-code">算法：Backtrack(t)
输入：当前处理到第 t 行
输出：4 皇后的所有可行解
if t &gt; 4 then
  输出一个解 x[1..4]
...
算法：Place(t)
for j ← 1 to t-1 do
  if x[j] = x[t] or |t-j| = |x[t]-x[j]| then
    return false
return true</pre>
<p>因此，4 皇后问题的求解过程可以概括为：逐行放置皇后，能放就继续向下一层搜索，发生冲突就剪枝并回溯。</p>`,
      answerImages: ["public/imgs/alg-4-8-search-tree.svg"]
    }),
    ref("9．用回溯算法解决旅游路线的问题。", "见标准答案", [], {
      answerHtml: `<p>旅游路线问题通常把一条路线表示为一个城市排列。若起点固定为 <code>X1</code>，则可把解写成 <code>(X1,X2,...,Xn)</code>，因此它的解空间结构是<strong>排列树</strong>。</p>
<p>从根结点开始，第一层选择出发后的第一个城市，第二层选择下一个未访问城市，依次向下扩展，直到形成一条完整路线。</p>
<p>回溯时常用两类判断：一类是约束函数，用来保证城市不重复且边存在；另一类是限界函数，用当前路径长度与已知最优值比较，若继续向下也不可能更优，则立即剪枝。</p>
<pre class="answer-code">算法：TSP-Backtrack(t)
if t &gt; n then
  计算回路长度
  更新当前最优解
else
  for i ← t to n do
    交换 x[t], x[i]
    if 当前部分路径可行且可能优于当前最优解 then
      Backtrack(t+1)
    交换 x[t], x[i]</pre>
<p>因此，这类问题的求解可以概括为：按排列树进行深度优先搜索，满足约束就向下扩展，不能优于当前最优解时就剪枝回溯。</p>`,
      answerImages: ["public/imgs/alg-4-9-route-tree.svg"]
    }),
    ref("10．回溯算法解决问题时对应解的定义是什么？解的组织结构有哪些？针对具体问题如何画解的组织结构，例如旅游路线问题的解组织结构，密码箱的解组织结构等。", "见标准答案", [], {
      answerHtml: `<p>回溯法中，一个解通常表示为一个 <strong>n 元组</strong> <code>x=(x1,x2,...,xn)</code>，其中每个分量表示一次决策。例如 0-1 背包中，<code>x[i]</code> 表示第 i 个物品取或不取；皇后问题中，<code>x[i]</code> 表示第 i 行皇后所在的列号。</p>
<p>解空间是所有可能解组成的空间。回溯法要按照某种组织结构在这个空间中进行搜索，这种组织结构通常画成<strong>解空间树</strong>。</p>
<p>常见的解空间组织结构有三种：</p>
<p>1．<strong>子集树</strong>：每层只有“取”或“不取”两种选择，适合 0-1 背包、最大团等问题。</p>
<p>2．<strong>排列树</strong>：每层从剩余元素中选择一个，适合旅行商、旅游路线、全排列等问题。</p>
<p>3．<strong>满 m 叉树</strong>：每个结点都有 m 个分支，适合每个位置都可重复取值的问题，如允许重复的密码箱问题。</p>
<p>具体判断时，旅游路线问题通常画成排列树；密码箱若每位数字不能重复，可画成排列树；若每位数字允许重复，则画成满 m 叉树。</p>`,
      answerImages: ["public/imgs/alg-4-10-structures.svg"]
    }),
    ref("11．已知序列 X=\"ABCDGHBB\" 和序列 Y=\"AEDFHRAB\"，当采用动态规划算法求解 X 和 Y 的最长公共子序列时，给出状态转移方程，并求 X 和 Y 的最长公共子序列长度。", "见标准答案", [], {
      answerHtml: `<p>设 <code>c[i][j]</code> 为 <code>X[1..i]</code> 与 <code>Y[1..j]</code> 的最长公共子序列长度，则状态转移方程为：</p>
<pre class="answer-code">当 i = 0 或 j = 0 时，c[i][j] = 0
若 Xi = Yj，则 c[i][j] = c[i-1][j-1] + 1
若 Xi ≠ Yj，则 c[i][j] = max(c[i-1][j], c[i][j-1])</pre>
<p>把两个序列代入表格计算后，可得它们的最长公共子序列长度为 <strong>4</strong>。一个最长公共子序列可以写成 <strong>ADHB</strong>。</p>`,
      answerImages: ["public/imgs/alg-4-11-state.png"]
    }),
    ref("12．小华有四个家 A、B、C、D，各个家之间都存在着路相连接，小华现在在家 B 游玩，想要将其他三个家都游玩一次，且每个家只去一次。问小华有多少种游玩的方案？画出该问题的解空间树。", "见标准答案", [], {
      answerHtml: `<p>从 B 出发后，剩余可访问地点为 A、C、D，共 3 个，并且每个家只去一次，所以方案数就是这 3 个地点的全排列数：</p>
<pre class="answer-code">3! = 3 × 2 × 1 = 6</pre>
<p>因此共有 <strong>6 种方案</strong>。该问题的解空间树属于<strong>排列树</strong>：根结点固定为 B，第一层从 A、C、D 中任选一个，第二层从剩余两个中选一个，第三层为最后一个未访问地点。</p>`,
      answerImages: ["public/imgs/alg-4-12-home-tree.svg"]
    })
  ]
};
...
function ref(text, answer, images = [], options = {}) {
  return { text, answer, images, ...options };
}
...
function renderReferenceQuestion(kind, index, item) {
  const id = `${kind}-${index}`;
  const alwaysVisible = kind === "algDesign";
  const visible = alwaysVisible || state.checked[id];
  ...
  return `
    <div class="question" id="card-${id}">
      <p class="question-title">${item.excluded ? `<span class="excluded-tag">排除题目</span>` : ""}${escapeHtml(item.text).replace(/\n/g, "<br>")}</p>
      ${images.map(src => `<figure class="question-media"><img src="${src}" alt="题目配图 ${index + 1}"></figure>`).join("")}
      ${actions}
      ${visible ? renderReferenceAnswer(item, index) : ""}
    </div>
  `;
}

function renderReferenceAnswer(item, index) {
  const answerImages = item.answerImages || [];
  const answerContent = item.answerHtml
    ? `<div class="answer-box rich-answer">${item.answerHtml}</div>`
    : `<div class="answer-box">${escapeHtml(item.answer).replace(/\n/g, "<br>")}</div>`;
  const images = answerImages.map(src => `<figure class="question-media answer-media"><img src="${src}" alt="参考图 ${index + 1}"></figure>`).join("");
  return `${answerContent}${images}`;
}
